...
class JInstallationModelConfiguration extends JModelLegacy
{
	...
	public function setup($options)
	{
        ...
        @$this->_refreshManifestCaches($options);
    ...
    function _refreshManifestCaches($options){
		...
		foreach($eids as $id) {
			$result|= $this->_doManifestRefresh((int)$id, $installer, $db);
		}
        return $result;
    }

    function _doManifestRefresh($id, &$installer, &$db)
    {
        if ($id) {
            ...
            if ($installer->extension->state == -1) {
                $installer->abort(JText::_('JLIB_INSTALLER_ABORT_REFRESH_MANIFEST_CACHE'));
                return false;
            }
