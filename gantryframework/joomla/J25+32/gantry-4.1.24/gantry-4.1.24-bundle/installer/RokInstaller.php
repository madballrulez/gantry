...
class RokInstaller extends JInstaller
{
	...
	{
		$adapters = array('component', 'file', 'language', 'library', 'module', 'plugin', 'template');
		$class    = "RokInstaller" . ucfirst($name);
		if (!class_exists($class)) {
			if (!is_object($adapter)) {
				foreach ($adapters as $rok_adapter) {
					// Try to load the adapter object
					$fullpath = dirname(__FILE__) . '/' . 'adapters' . '/' . strtolower($rok_adapter) . '.php';
	...
	function prepExceptions($element, $cid = 0)
	{
		...
		$this->backup_dir = $config->getValue('config.tmp_path') . '/' . uniqid('backup_');
	...
	function copyFiles($files, $overwrite = null)
	{
		...
		// try to make writeable   get Overwrite for J1.6/1.7/2.5  isOverwrite for J3
		if ($overwrite || (method_exists($this,'getOverwrite') && $this->getOverwrite()) || (method_exists($this,'isOverwrite') && $this->isOverwrite())) {
			foreach ($files as $file) {
